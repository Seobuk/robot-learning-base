...
- **프로젝트 온보딩(setup)**: 기존 코드를 프로젝트에 넣고 `이 코드 세팅해줘`/`기존 코드 분석해서 CLAUDE.md 만들어줘`처럼 요청하면 트리거. 의존성 설치 → 코드 분석 → **개발환경 가이드 + 메모리 MD + 코드에 맞는 CLAUDE.md** 생성. 설치·파일 생성/덮어쓰기 등 **변경 전엔 항상 먼저 물어본다**(기존 CLAUDE.md는 clobber 없이 merge).
